...
function Header() {
    ...
    return (
        <div className="w-full z-50 fixed flex items-center  justify-around border-b-2 bg-white h-20">
            {searchBar && searchResponse && <div class="absolute mt-20    w-full inset-y-0  flex justify-center  pr-2">
        <div className='bg-white overflow-y-scroll  h-96 w-2/5  border-2  rounded-b-lg'>
        {searchResponse?.map((Branch, index) => {
        return(
            <div onClick={()=>{history.push("/company/"+ Branch.CompanyName +"/" + Branch.BranchName);window.location.reload()}} className="cursor-pointer w-full flex flex-col justify-center hover:bg-gray-100 bg-white border-2 h-20 text-black">
        ...
        <div className="container h-3/4 p-3 flex items-center ">
            ...
            <p className="ml-auto left-full">{Branch.CompanyName} {Branch.BranchAddressCity} / {Branch.BranchAddressdistrict} Şubesi </p></div>
        </div> 
        )
    })
        }
  ...
  <input   ref={searchText} class="w-full h-10 pl-8 pr-3 text-base placeholder-gray-600 border rounded-lg outline-none bg-gray-200 focus:border-black" onChange={()=>handleSearch()}  onClick={()=>{setSearchBar(true)}} type="text" placeholder="Şubeye Göre Arama Yap"/>
